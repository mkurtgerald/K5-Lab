...
Prediction receipts explicitly carry `authorized=false`. Learned scores are not described as calibrated probabilities. Unlabeled inputs can be scored but cannot update model or temporal state. The public adapter exposes no model/plugin selector, remote resource, persistence format, execution endpoint, or external action.
...
The fixture is synthetic and generic. It is not evidence of real-world sensor intelligence, calibration, safety, or production quality. No weights or records are saved.

## Remaining S2 exit work

Before S2 can close, the candidate still requires exact hosted artifact/native-library capture, state reload/corruption handling, stronger held-out drift assertions, explicit memory-growth evidence, and exact-head CI on the complete S2 acceptance set. Commercial distribution remains false until SBOM/notices/vulnerability/platform review is complete.
