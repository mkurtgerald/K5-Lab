...
The synthetic benchmark reports insert/query/correlation p50/p95 latency and aggregate operations-per-second observations, plus Python-side peak allocation at multiple configured event-count bounds. It also exercises deterministic replay, bounded count eviction, missing/cross-partition references, oversized query windows, missing correlation references, and too-late input as expected-negative paths. These measurements are regression evidence only; `tracemalloc` does not establish native/process memory qualification and the fixture is not a production-scale claim.
